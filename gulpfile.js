...
/**
 * Run a run, watch, and default task using the parallel function so it can
 ...
 */
gulp.task('run', gulp.parallel('sass', 'js', 'images'));
...
function watchSassTask() {
  return gulp.watch(srcFiles.scss, function() {
    return sassTask();
  });
}

function watchJsTask() {
  return gulp.watch(srcFiles.js, function() {
    return jsTask();
  });
}

function watchImagesTask() {
  return gulp.watch(srcFiles.images, function() {
    return imagesTask();
  });
}
